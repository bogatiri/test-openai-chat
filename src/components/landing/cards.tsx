		...
		<div>
			<div className='w-full px-10 sm:px-20 md:px-20 lg:px-20 xl:px-32 2xl:px-40 3xl:px-60 4xl:px-80 grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-6'>
				{cards.map((card, index) => (
					...
					>
						<h3 className='text-md lg:text-xl font-bold text-center lg:text-start'>{card.title}</h3>
						<p className='text-xs lg:text-md text-center lg:text-start h-full'>{card.body}</p>
					</div>
				))}
			</div>
		</div>
